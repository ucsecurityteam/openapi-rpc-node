...
const LoggingMetricConstants = require('../logging/logging_metric_constants');
...
function initMonitoringClient(serviceId, RPCFramework) {
  const defaultLabels = { 
  ...
  try {
    ...
    Mycroft.registerMetric.counter(RPC_METRICS.STORE, {
      ...
      labelNames: [LoggingMetricConstants.LABEL.STATUS_CODE,
        LoggingMetricConstants.LABEL.ERROR_TYPE,
        LoggingMetricConstants.LABEL.LOG_TYPE]
    });
...
function initMiddlewareMetric(defaultLabels) {
...
function initAPIKeyTrackingMetric() {
...
function initApplicationMonitoringClient(params) {
...
function validateConfig(metric) {
...
function logError(errType, errMessage) {
  let logData = {};
...
module.exports = {
  ...
  initApplicationMonitoringClient
}
