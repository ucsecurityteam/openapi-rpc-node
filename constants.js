...
const { getNodeVersion } = require('./common/service_metadata_util');
var _ = require('lodash');
const ConfigUtils  = require('./common/config_utils');

let globalConfig;

function getGlobalRpcConfig(constantsType, path) {
  if(_.isUndefined(globalConfig)) {
...
let constants = {
  ...
  DEPENDENCY: {
    ...
    CONFIG_PATH: '/configs/dependency.config',
  ...
  CLIENT: {
    ...
    HTTP_AGENT_DEFAULT_OPTIONS: {
      keepAlive: getNodeVersion() >= 14 ? true : false,
      maxFreeSockets: 10,
